...
func (cs *CoinSet) TotalValue() (value fakutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue fakutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue fakutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount fakutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue fakutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue fakutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue fakutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue fakutil.Amount, coins []Coin) (Coins, error) {
...
// fakutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() fakutil.Amount {
	return fakutil.Amount(c.txOut().Value)
}
